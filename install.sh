...
function systemdpart3 {
	...
	ls | grep -w "encrypt" && echo "options rd.luks.name='$ESP4'=cryptroot root=/dev/mapper/cryptroot" | cat >> /mnt/boot/loader/entries/arch.conf || echo "options root="'"'UUID="$ESP4"'"' " rw " | cat >> /mnt/boot/loader/entries/arch.conf
}
function pewpew {
