...
function home_set {
	ls | grep -w "encrypt_for_home" && formathome || installhome
}
...
function create_home_entry {
	...
	ls | grep -w "zlib" && echo "Options=compress-force=zlib" >> home.mount
	ls | grep -w "lzo" && echo "Options=compress-force=lzo" >> home.mount
	ls | grep -w "zstd" && echo "Options=compress-force=zstd" >> home.mount
	ls | grep -w "nocom" && echo " " >> home.mount
