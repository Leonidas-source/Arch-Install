...
function systemdpart3 {
	...
	ls | grep -w "encrypt" && echo "options rd.luks.name="$real"=root_partition root="'"'UUID="$arg"'"' " rw $var" | cat >> /mnt/boot/loader/entries/arch.conf
	ls | grep -w "encrypt" || echo "options root="'"'UUID="$real"'"' " rw $var" | cat >> /mnt/boot/loader/entries/arch.conf
}
systemd
