...
function secure {
  ...
  read -e decrypted
  red=$(lsblk -f $decrypted -o UUID | sed s/"UUID"/""/g | sed '/^$/d;s/[[:blank:]]//g')
  ls | grep LINUX && efibootmgr --disk $ESP --part $ESP2 --create --label "Arch" --loader  /vmlinuz-linux  --unicode 'rd.luks.name='$ESP4'=cryptroot root=UUID="$red" rw initrd=\initramfs-linux.img'
  ls | grep ZEN && efibootmgr --disk $ESP --part $ESP2 --create --label "Arch_Zen" --loader  /vmlinuz-linux-zen  --unicode 'rd.luks.name='$ESP4'=cryptroot root=UUID="$red" rw initrd=\initramfs-linux-zen.img'
  ls | grep LTS && efibootmgr --disk $ESP --part $ESP2 --create --label "Arch_Lts" --loader  /vmlinuz-linux-lts  --unicode 'rd.luks.name='$ESP4'=cryptroot root=UUID="$red" rw initrd=\initramfs-linux-lts.img'
  ls | grep HARD && efibootmgr --disk $ESP --part $ESP2 --create --label "Arch_Hard" --loader  /vmlinuz-linux-hardened  --unicode 'rd.luks.name='$ESP4'=cryptroot root=UUID="$red" rw initrd=\initramfs-linux-hardened.img'
}
function regular {
  clear
